...
public class LoginServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(rememberMe != null) {
            cUsername.setMaxAge(60*60*24*30); //1 month
            cPassword.setMaxAge(60*60*24*30);
            cRememberMe.setMaxAge(60*60*24*30);
        } else {
        ...
        Account a = ad.check(username, password);
        if(a == null) {
            request.setAttribute("error", "Username or password is invalid!");
            request.getRequestDispatcher("login.jsp").forward(request, response);
        } else {
            HttpSession session = request.getSession();
            session.setAttribute("displayname", a.getDisplayname());
